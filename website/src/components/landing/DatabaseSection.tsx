...
import { Database, ShieldAlert, Table, Command, ArrowRight, Copy, Check, TriangleAlert } from "lucide-react"
import { Link } from "react-router-dom"
import { useTranslation } from "react-i18next"

// DB_PROMPTS will be generated in the component to access translations
...
export function DatabaseSection() {
  ...
  const DB_PROMPTS = [
    {
      name: "/running_query (MySQL)",
      desc: t('database.prompts.runMysql'),
      cmd: '/running_query "Run query to find top 5 customers with status active."'
    },
    {
      name: "/auditor_query (MySQL)",
      desc: t('database.prompts.auditMysql'),
      cmd: '/auditor_query "Analyze query performance for SELECT * FROM items WHERE status = pending"'
    },
    {
      name: "/running_pg_query (PostgreSQL)",
      desc: t('database.prompts.runPg'),
      cmd: '/running_pg_query "Show all tables in public schema."'
    },
    {
      name: "/auditor_pg_query (PostgreSQL)",
      desc: t('database.prompts.auditPg'),
      cmd: '/auditor_pg_query "Analyze query performance for SELECT * FROM users"'
    }
  ]

  return (
    <section className="py-24 bg-background transition-colors w-full border-b border-border/40" id="database">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="flex flex-col items-start text-left lg:sticky lg:top-24">
            ...
            <div className="p-5 rounded-2xl border border-blue-500/20 bg-blue-500/5 flex items-start gap-4 mb-4 w-full">
              ...
              <div>
                <h4 className="font-bold text-foreground text-sm m-0">{t('database.strictReadOnly')}</h4>
                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed m-0">
                  {t('database.strictReadOnlyDesc')}
                </p>
              </div>
            </div>

            {/* Production Warning Card */}
            <div className="p-5 rounded-2xl border border-amber-500/20 bg-amber-500/5 flex items-start gap-4 mb-10 w-full">
              <TriangleAlert className="h-6 w-6 text-amber-500 mt-0.5 shrink-0" />
              <div>
                <h4 className="font-bold text-foreground text-sm m-0">{t('database.notForProd')}</h4>
                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed m-0">
                  {t('database.notForProdDesc')}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-row flex-wrap gap-3">
              ...
              >
                {t('database.docs')} <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
            ...
            {DB_PROMPTS.map((tool, idx) => (
              <div key={idx} className="p-6 rounded-2xl border border-border bg-card/60 backdrop-blur-sm flex flex-col gap-4 transition-all hover:bg-card/80 hover:shadow-sm">
                ...
                <div className="pt-3 border-t border-border/40 text-xs flex flex-col gap-3">
                  <span className="text-[10px] uppercase font-extrabold text-muted-foreground tracking-wider block mb-1">{t('database.examplePrompt')}</span>
